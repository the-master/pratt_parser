			...
			if (is_number(root->val))
				...
				//	print_all(function_argument);
					do {//TODO  fix
						int temp;
						print_Ast(root);
						set_value(function_scope, tr->right->left->val, eval_Ast(tr->right->right, context));
						tr = tr->left;
	...
	case no_left_operand:
		...
		default:
			if (is_a_basetype_number(root->val))
				...
				if (f->arg_count == 1)
				{
					set_value(function_scope, root->left->left->val, eval_Ast_temp(root->left->right, context).i);
				}
				else {
					AbstractSyntaxTree* tr = root->left;
					//	TokenStream function_argument = function_arguments(tr);
					...
					do {//TODO  fix
						int temp;
						print_Ast(root);
						set_value(function_scope, tr->right->left->val, eval_Ast_temp(tr->right->right, context).i);
						tr = tr->left;
					} while (tr && tr->val == statement_seperator);

					set_value(function_scope, tr->left->val, eval_Ast_temp(tr->right, context).i);
				}
				//	function_scope->up = 0;
					//print_Ast(f->tree);
				...
				if (function_scope->alive)
					...
					return (value_container) { int_b, function_scope->result };
			}
			else
				return (value_container) { int_b, * root->val == '-' ? -get_value(context, root->val + 1) : get_value(context, root->val) };
		}
	case with_left_operand:
		switch ((int)root->val) {
		case	plus: 
		...
		case	assign:
			if (root->left && root->left->val == declaration)
				return (value_container) { int_b, set_value(context, root->left->right->val, eval_Ast_temp(root->right, context).i) };

			else if (root->left)
				return (value_container) { int_b, set_value(context, root->left->val, eval_Ast_temp(root->right, context).i) };
			else
		...
		case	declaration:return(value_container) {int_b, set_value(context, root->right->val, 0)	};
